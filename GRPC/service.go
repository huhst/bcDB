...
func (s *Service) DistributeDataBlock(ctx context.Context, req *BcGrpc.DataBlock) (info *BcGrpc.VerifyInfo, err error) {
	...
	flag := localDataBlockChain.CheckDataBlock(block)
	if !flag {
		info.Info = "区块校验失败"
		info.Status = false
	}
	// 将区块上链
	localDataBlockChain.AddBlockToChain(*block)
	// 交易池更新
	localTxPool.UpdateByData(*block)
	// 缓存更新
	localCache.UpdateByDataBlock(*block)
	return info, nil
}
...
func (s *Service) DataBlockSynchronization(ctx context.Context, req *BcGrpc.ReqDataBlock) (out *BcGrpc.ResDataBlocks, err error) {
	...
	// 获得迭代器
	iterator := localDataBlockChain.CreateIterator()
	// 获得未同步的区块
	for {
		block := iterator.Next()
...
func (s *Service) DistributeTableBlock(ctx context.Context, req *BcGrpc.TableBlock) (info *BcGrpc.VerifyInfo, err error) {
	...
	flag := localTableBlockChain.CheckTableBlock(block)
	if !flag {
		info.Info = "区块校验失败"
		info.Status = false
	}
	// 上链
	localTableBlockChain.AddBlockToChain(*block)
	// 交易池更新
	localTxPool.UpdateByTable(*block)
	// 缓存更新
	localCache.UpdateByTableBlock(*block)
	return info, nil
}
...
func (s *Service) TableBlockSynchronization(ctx context.Context, req *BcGrpc.ReqTableBlock) (out *BcGrpc.ResTableBlocks, err error) {
	...
	// 获得迭代器
	iterator := localTableBlockChain.CreateIterator()
	// 获得未同步的区块
	for {
		block := iterator.Next()
		if !bytes.Equal(block.CurrentBlockHash, req.Hash) {
...
func (s *Service) JoinCluster(ctx context.Context, req *BcGrpc.ReqJoin) (info *BcGrpc.VerifyInfo, err error) {
	...
	if !bytes.Equal(req.JoinKey, cluster.Key) {
		log.Println(req.JoinKey, cluster.Key)
		info.Info = "加入集群的密钥错误"
		info.Status = false
...
func (s Service) Handle(ctx context.Context, data *BcGrpc.TypAndData) (info *BcGrpc.Info, err error) {
	info = &BcGrpc.Info{}
	// 进行处理
	go algorand.LocalPeer.Handle(int(data.Typ), data.Data)
	return info, nil
}
