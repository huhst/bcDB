...
func (q *tableQueue) in(transaction blockchain_table.Transaction) {
	//log.Println("tableQueue in begin!")
	p := &tableTxNode{
		tx:   transaction,
		pre:  nil,
		next: nil,
...
func (tpl *TxPoolTable) bookKeeperRun() {
	...
	for tpl.count >= tpl.packNumber {
		...
		// 进行algorand过程
		// 1. 将区块传入同步队列
		blockqueue.TableBlock2Algorand.Put(block)
		//table test1 1FYrDeBmc1yLUkTFuJEtjFpRjjhWQdqspE4
		//tpl.OrdinaryRun(block)
		for {
			time.AfterFunc(10*time.Second, func() {
				// 2. 将算法输出的区块的内容与传入的区块进行合并
				if blockqueue.Algorand2TableBlock.Size >= 1 {
					log.Println("权限区块上链，bcdb")
					get := blockqueue.Algorand2TableBlock.Get()
